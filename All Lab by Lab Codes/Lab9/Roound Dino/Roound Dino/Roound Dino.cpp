...
void myDisplay(void) {
    ...
    for (int i = 0; i < numDinos; i++) {
        double angle = 2.0 * 3.14159265359 * i / numDinos; // Calculate angle for each dino
        double x = radius * cos(angle);
        double y = radius * sin(angle);

        glPushMatrix();
        glTranslatef(screenWidth / 2 + x, screenHeight / 2 + y, 0);
        glScalef(0.1, 0.1, 1.0);  // Scale down the size by a factor of 0.5
